from pylab import *
from init_door_config import calc_transitions, place_reward, rooms, transitions, doors, neighbours, n_rooms, n_doors
from maze_functions import calc_choice_options_NESW, calc_opendoors, calc_agent_direction, calc_door_direction, calc_door_position_NESW, check_door_flip, coord_transform, calc_room_successor_value, read_door_options_NESW, update_doors_with_log
from model_fitting import softmax_array, nll, update_nll, calc_lrandom
from log_info import col, line

from planning import init_belief, calc_Q_plan
...
def calc_model_fit(params, fixed_params):    
	...
	P_belief = init_belief(rooms, n_rooms)
	...
	print "params (beta, gamma, eta) = ", params
	...
	#for i in range(28): # first data line = line 27
		if len(fl[i].split()) > 18 and i>line['descr']: # Excluding header lines
			...
			counter = 0
			while counter < 1: #2: # I seem to need this for correctly updating the door state
				counter += 1

				#successor_rep_neighbours = calc_SR(neighbours, rooms['reward'], SR_lambda) # 0.99
              

				#room_successors[curr_trial, :, i_subj], room_successor_value[curr_trial, :, i_subj], choice_options_NESW[curr_trial, :, i_subj], open_doors_calc_index = calc_room_successor_value(choice_NESW[:, i_subj], doors, room_at_decision[:, i_subj], avail_doors, successor_rep_neighbours, curr_trial) #

				open_doors_calc_index = calc_opendoors(doors, room_at_decision[:, i_subj], avail_doors, curr_trial)
				...
				open_door_index = calc_opendoors(doors, room_at_decision[:, i_subj], avail_doors, curr_trial)
				Q_plan = calc_Q_plan(int(room_at_decision[curr_trial, i_subj]), open_door_index, i_atDoor_current, P_belief, gamma, eta) # TODO: Implement the planning function which calculates "values resulting from planning" on-the-fly!

			#print "choice_options_NESW[curr_trial, :, i_subj] = ", choice_options_NESW[curr_trial, :, i_subj]
			#print "choice_NESW[curr_trial, i_subj] = ", choice_NESW[curr_trial, i_subj]

			#print "Q_plan.max(1) = ", Q_plan.max(1)
			#print "Q_plan.shape = ", Q_plan.shape

			nll_0, ptot_0 = update_nll(nll_0, ptot_0, beta_inp, [i_room_current], Q_plan.max(1), choice_options_NESW[curr_trial, :, i_subj], choice_NESW[curr_trial, i_subj]) # ??? How to treat Q_plan ???
...
if __name__ == '__main__':
	...
	filename_out = 'fit_data/planning.txt' 
	...
	for str_AB in ['A', 'B']:
		for str_YO in ['Y', 'O']:
			...
			for i_subj in xrange(n_subjects):
				...
				param_bounds = ((0, None), (0, 1), (0, 1))
				guess_beta = 1.0
				guess_gamma = 0.8
				guess_eta = 0.14
				x0 = [guess_beta, guess_gamma, guess_eta]                   

				res = sciopt.minimize(calc_model_fit, x0, [fl, i_subj], bounds=param_bounds) 
				#NLL_array[i_subj] = calc_model_fit(x0, [fl, i_subj])#, guess_lambda]) 
				#beta[i_subj] = guess_beta
				#gamma[i_subj] = guess_gamma
				#eta[i_subj] = guess_eta

				NLL_array[i_subj] = calc_model_fit(res.x, [fl, i_subj])#, guess_lambda])                   
				beta[i_subj] = res.x[0]
				gamma[i_subj] = res.x[1]
				eta[i_subj] = res.x[0]
				...
				str_out = "Subject; " + str(id_list[i_subj]) + '_' + str_AB +"; "+ "Model_plan; " + "beta; " + '%.2f'%(beta[i_subj]) + "; gamma; "+ '%.2f'%(gamma[i_subj])+ "; eta; " + '%.2f'%(eta[i_subj])  + "; NLL; "+ '%.2f'%(NLL_array[i_subj]) + "; " + "rho_square; " + '%.2f'%(rho_square) + "; " + res.message + "\n"                                                                
				print str_out
